...
with fluid.dygraph.guard(place):
    ...
    optimizer1=fluid.optimizer.SGDOptimizer(learning_rate=0.01,parameter_list=model1.parameters(),regularization=fluid.regularizer.L2Decay(regularization_coeff=0.1))
    optimizer2 = fluid.optimizer.AdamOptimizer(learning_rate=0.01, parameter_list=model2.parameters(),
...
for i in range(len(acc_lists)):
    ...
    plt.plot(acc_lists[i],label="acc_"+names[i])
    plt.plot(loss_lists[i], label="loss_" + names[i])
plt.legend()
plt.show()
